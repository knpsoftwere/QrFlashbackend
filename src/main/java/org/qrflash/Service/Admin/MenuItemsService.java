...
/*    public List<MenuItemDTO> getMenuItems(String databaseName) throws SQLException {
        String query = """
        SELECT mi.id AS menu_item_id, mi.name, mi.description, mi.unit, mi.item_type, mi.price, mi.is_active,
               c.id AS category_id, c.name AS category_name, c.description AS category_description, c.image_url AS category_image_url,
               t.id AS tag_id, t.name AS tag_name, t.description AS tag_description, t.emoji AS tag_emoji
        FROM menu_items mi
        LEFT JOIN categories c ON mi.category_id = c.id
        LEFT JOIN menu_item_tags mit ON mi.id = mit.menu_item_id
